...
public class XSaLTPDFTools
{
	...
	//		}
	//
	//		PdfStamper oMyStamper = new PdfStamper(oReader, new FileOutputStream("C:/Modified.pdf"));
	...
	public void showTextAligned(int alignment, String text, float x, float y, float rotation)
	{
		...
		if (text.indexOf("$") != -1 && alignment == ALIGN_RIGHT && text.endsWith(" CR"))
		{
			showTextAligned(alignment, text.substring(0, text.length() - 3), x, y, rotation, false);
			showTextAligned(ALIGN_LEFT, "CR", x + 4, y, rotation, false);
		}
		else
		{
		...
		try
		{
			Rectangle oPageSize = PageSize.LETTER;
			XSaLTPDFTools ioPDFTools = new XSaLTPDFTools("C:/bobo.pdf", oPageSize);
			ioPDFTools.ioPdfDocument.newPage();
			String sTextCopy = "<table><tr><td>This is Liz's customized message center that shows <B>bold</B>, <I>italic</I>, <U>underline</U> and <B><I>bold italic</I></B>.</td></tr></tbody></table>";
			ioPDFTools.generateFormattedMessageCenter(0, 0, oPageSize.getWidth(), oPageSize.getHeight(), sTextCopy, false, false, true);
	...
	try
		{
			Rectangle oPageSize = PageSize.LEGAL;
			XSaLTPDFTools ioPDFTools = new XSaLTPDFTools("C:/TestSize.pdf", oPageSize);

			ioPDFTools.ioPdfDocument.setPageSize(PageSize.LETTER);
			ioPDFTools.ioPdfDocument.newPage();
			ioPDFTools.getPDFContentByte().beginText();
			ioPDFTools.getPDFContentByte().setFontAndSize(ioPDFTools.ioBaseFontHelveticaBold, 10);
			ioPDFTools.getPDFContentByte().showTextAligned(PdfContentByte.ALIGN_CENTER, "This should print on LETTER sized paper.",
					ioPDFTools.ioPdfDocument.getPageSize().getWidth() / 2, ioPDFTools.ioPdfDocument.getPageSize().getHeight() / 2, 0);
			...
			ioPDFTools.getPDFContentByte().beginText();
			ioPDFTools.getPDFContentByte().setFontAndSize(ioPDFTools.ioBaseFontHelveticaBold, 10);
			ioPDFTools.getPDFContentByte().showTextAligned(PdfContentByte.ALIGN_CENTER, "This should print on LEGAL sized paper.",
					ioPDFTools.ioPdfDocument.getPageSize().getWidth() / 2, ioPDFTools.ioPdfDocument.getPageSize().getHeight() / 2, 0);
			ioPDFTools.getPDFContentByte().endText();
	...
	*/
	 ...
	 * 
	 * @param _sPDFFilePath
	 *            Path to create PDF file
	 * @param _oPageSize
	 *            Rectangle object to determine page size
	...
	public void setStandardBaseFonts() throws DocumentException, IOException
	{
		...
		ioBaseFontHelveticaBoldItalic = BaseFont.createFont(BaseFont.HELVETICA_BOLDOBLIQUE, BaseFont.WINANSI, BaseFont.NOT_EMBEDDED);
		ioBaseFontHelveticaItalic = BaseFont.createFont(BaseFont.HELVETICA_OBLIQUE, BaseFont.WINANSI, BaseFont.NOT_EMBEDDED);
		...
		ioBaseFontTimesBoldItalic = BaseFont.createFont(BaseFont.TIMES_BOLDITALIC, BaseFont.WINANSI, BaseFont.NOT_EMBEDDED);
		ioBaseFontTimesItalic = BaseFont.createFont(BaseFont.TIMES_ITALIC, BaseFont.WINANSI, BaseFont.NOT_EMBEDDED);

		//		ioBaseFontDingbats = BaseFont.createFont(BaseFont.ZAPFDINGBATS, BaseFont.WINANSI, BaseFont.NOT_EMBEDDED);
		...
		//		ioFontDingbatsRegular_6 = new Font(Font.getFamily("Helvetica"), 6, Font.NORMAL);
		//		ioFontDingbatsBold_6 = new Font(Font.getFamily("Helvetica"), 6, Font.BOLD);
		//		ioFontDingbatsBoldItalic_6 = new Font(Font.getFamily("Helvetica"), 6, Font.BOLDITALIC);
		//		ioFontDingbatsItalic_6 = new Font(Font.getFamily("Helvetica"), 6, Font.ITALIC);
	 ...
	 * 
	 * @param _sBackgroundImageFilePath
	 *            File path to get image
	 * @param _fShimmyLeftRight
	 *            Absolute X-coordinate to anchor image at
	 * @param _fShimmyUpDown
	 *            Absolute Y-coordinate to anchor image at
	 *            
	 * @throws MalformedURLException
	 * @throws IOException
	 * @throws DocumentException
	 */
	public void setBackgroundImage(String _sBackgroundImageFilePath, float _fShimmyLeftRight, float _fShimmyUpDown) throws MalformedURLException, IOException, DocumentException
	{
	...
	public void createLeftText(PdfContentByte _oPDFContentByte, String _text, BaseFont _baseFont, float _fontSize, float _fPositionX, float _fPositionY) {
	...
	public void createCenterText(PdfContentByte _oPDFContentByte, String _text, BaseFont _baseFont, float _fontSize, float _fPositionX, float _fPositionY) {
		_oPDFContentByte.beginText();
		_oPDFContentByte.setFontAndSize(_baseFont, _fontSize);
	...
	public void createRightText(PdfContentByte _oPDFContentByte, String _text, BaseFont _baseFont, float _fontSize, float _fPositionX, float _fPositionY) {
		_oPDFContentByte.beginText();
		_oPDFContentByte.setFontAndSize(_baseFont, _fontSize);
	 ...
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _sLeftSideDisplayText
	 *            Label text
	 * @param _oLeftBaseFont
	 *            Font for label text
	 * @param _fLeftFontSize
	 *            Font size for label text
	 * @param _sRightSideDisplayText
	 *            Value text
	 * @param _oRightBaseFont
	 *            Font for value text
	 * @param _fRightFontSize
	 *            Font size for value text
	 * @param _fPositionX
	 *            X-coordinate to render text
	 * @param _fPositionY
	 *            Y-coordinate to render text
	 */
	public void createRightLeftColonValue(PdfContentByte _oPDFContentByte, String _sLeftSideDisplayText, BaseFont _oLeftBaseFont, float _fLeftFontSize,
			String _sRightSideDisplayText, BaseFont _oRightBaseFont, float _fRightFontSize, float _fPositionX, float _fPositionY)
	{

		_oPDFContentByte.beginText();
		_oPDFContentByte.setFontAndSize(_oLeftBaseFont, _fLeftFontSize);
		_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_RIGHT, _sLeftSideDisplayText, _fPositionX, _fPositionY, 0);
		_oPDFContentByte.setFontAndSize(_oRightBaseFont, _fRightFontSize);
		_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sRightSideDisplayText, _fPositionX + (_fLeftFontSize / 2), _fPositionY, 0);
		_oPDFContentByte.endText();
	}

	/**
	 * This method renders two sets of text on a line, the left text will be
	 * aligned left and the right text will be aligned right.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _sLeftSideDisplayText
	 ...
	 *            Font for left side
	 * @param _fLeftFontSize
	 *            Font size for left side
	 * @param _sRightSideDisplayText
	 *            Text for right side
	 * @param _oRightBaseFont
	 *            Font for right side
	 * @param _fRightFontSize
	 *            Font size for right side
	 * @param _fPositionX1
	 *            X-coordinate for left text
	 * @param _fPositionX2
	 *            X-coordinate for right text
	 * @param _fPositionY
	 *            Y-coordinate for created text set
	 * @throws DocumentException
	 */
	public void createRightLeftValue(PdfContentByte _oPDFContentByte, String _sLeftSideDisplayText, BaseFont _oLeftBaseFont, float _fLeftFontSize, String _sRightSideDisplayText,
			BaseFont _oRightBaseFont, float _fRightFontSize, float _fPositionX1, float _fPositionX2, float _fPositionY) throws DocumentException
	{
		_oPDFContentByte.beginText();
		_oPDFContentByte.setFontAndSize(_oLeftBaseFont, _fLeftFontSize);
		_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sLeftSideDisplayText, _fPositionX1, _fPositionY, 0);
		float fLeftPhraseWidth = _oPDFContentByte.getEffectiveStringWidth(_sLeftSideDisplayText, false) + _fLeftFontSize / 2;
		_oPDFContentByte.setFontAndSize(_oRightBaseFont, _fRightFontSize);
		_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_RIGHT, _sRightSideDisplayText, _fPositionX2, _fPositionY, 0);
		float fRightPhraseWidth = _oPDFContentByte.getEffectiveStringWidth(_sRightSideDisplayText, false) + _fRightFontSize / 2;
	 ...
	 *            Font for left text
	 * @param _fLeftFontSize
	 *            Font size for left text
	 * @param _sRightSideDisplayText
	 *            Text for right side
	 * @param _oRightBaseFont
	 *            Font for right text
	 * @param _fRightFontSize
	 *            Font size for right text
	 * @param _oLeaderFont
	 *            Font for leader
	 * @param _fPositionX1
	 *            X-coordinate for left text
	 ...
	 *            X-coordinate for right text
	 * @param _fPositionY
	 *            Y-coordinate for text line
	 * @throws DocumentException
	 */
	public void createRightLeftLeaderValue(PdfContentByte _oPDFContentByte, String _sLeftSideDisplayText, BaseFont _oLeftBaseFont, float _fLeftFontSize,
			String _sRightSideDisplayText, BaseFont _oRightBaseFont, float _fRightFontSize, Font _oLeaderFont, float _fPositionX1, float _fPositionX2, float _fPositionY)
			throws DocumentException
	{
		...
		{
			fRightPhraseWidth = _oPDFContentByte.getEffectiveStringWidth(_sRightSideDisplayText.substring(0, _sRightSideDisplayText.length() - 3), false) + _fRightFontSize / 2;
		}
		else
		{
			fRightPhraseWidth = _oPDFContentByte.getEffectiveStringWidth(_sRightSideDisplayText, false) + _fRightFontSize / 2;
		}
		...
		oColumnText.setSimpleColumn(_fPositionX1 + fLeftPhraseWidth, _fPositionY + _oLeaderFont.getCalculatedSize(), _fPositionX2 - fRightPhraseWidth, _fPositionY,
	 ...
	 *            PDF object where content will be rendered
	 * @param _fPositionY
	 *            Y-coordinate for columns to be rendered
	 * @param _abfFonts
	 *            Array of fonts for text
	 * @param _anFontSizes
	 *            Array of font sizes for text
	 * @param _anAlignments
	 *            Array of alignment values
	 * @param _asTexts
	 *            Array of text to display in columns
	 * @param _afXPositions
	 *            Array of x-coordinates for columns
	 */
	public void createColumns(PdfContentByte _oPDFContentByte, float _fPositionY, BaseFont[] _abfFonts, int[] _anFontSizes, int[] _anAlignments, String[] _asTexts,
			float[] _afXPositions)
	{
		_oPDFContentByte.beginText();
	...
	/**
	 * This method creates and writes columns in a PDF document based on each columns percentage of the total line width.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _fPositionY
	 ...
	 * @param _fLineWidth
	 *            Total line width
	 * @param _fLeftPosition
	 *            X-position for left of line
	 * @param _aoFonts
	 *            Array of fonts for text
	 * @param _anFontSizes
	 *            Array of font sizes for text
	 * @param _anAlignments
	 *            Array of alignments for text
	 * @param _asTexts
	 *            Array of texts
	 * @param _afColumnWidths
	 *            Array of percents of line width for each column (in decimal format)
	 */
	public void createColumnsByLineWidthPct(PdfContentByte _oPDFContentByte, float _fPositionY, float _fLineWidth, float _fLeftPosition, BaseFont[] _aoFonts, int[] _anFontSizes,
			int[] _anAlignments, String[] _asTexts, float[] _afColumnWidths)
	{
		_oPDFContentByte.beginText();
	 ...
	 *            Array of x-coordinates for columns
	 * @param _oLeaderFont
	 *            Font for leader (same for all columns)
	 * @throws DocumentException
	 */
	public void createColumnsWithLeaders(PdfContentByte _oPDFContentByte, float _fPositionY, BaseFont[] _abfFonts, int[] _anFontSizes, int[] _anAlignments, String[] _asTexts,
			float[] _afXPositions, Font _oLeaderFont) throws DocumentException
	{
		...
		for (int i = 0; i < nNumber; i++)
		{
			...
			{
				fCurrentItemTabPositionRightEdge = _oPDFContentByte.getEffectiveStringWidth(_asTexts[i], false) + _anFontSizes[i] + _afXPositions[i];
			...
			else if (_anAlignments[i] == Element.ALIGN_RIGHT)
			{
				...
				fCurrentItemTabPositionLeftEdge = _afXPositions[i] - _oPDFContentByte.getEffectiveStringWidth(_asTexts[i], false) - _anFontSizes[i];
			}
			else
			{
				fCurrentItemTabPositionRightEdge = (_oPDFContentByte.getEffectiveStringWidth(_asTexts[i], false) / 2) + _anFontSizes[i] + _afXPositions[i];
				fCurrentItemTabPositionLeftEdge = _afXPositions[i] - (_oPDFContentByte.getEffectiveStringWidth(_asTexts[i], false) / 2) - _anFontSizes[i];
			}
			...
			if (i > 0)
			{
				...
				oPhrase.add("................................................................................................................................................................................................................................................................................................................................................................................................................................................................................................");
				oColumnText.addText(oPhrase);
				oColumnText.setSimpleColumn(fLastItemTabPositionRightEdge, _fPositionY + _oLeaderFont.getCalculatedSize(), fCurrentItemTabPositionLeftEdge, _fPositionY,
						_oLeaderFont.getSize(), Element.ALIGN_LEFT);
				oColumnText.go();
	...
	/**
	 * This method renders three text values, the first aligned left and the
	 * next two aligned right with the desired coordinates and fonts.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _sLeftSideDisplayText
	 ...
	 * @param _fLeftFontSize
	 *            Font size for left side
	 * @param _sRightSide1DisplayText
	 *            1st text for right side
	 * @param _oRight1BaseFont
	 *            Font for 1st right text
	 * @param _fRight1FontSize
	 *            Font size for 1st right text
	 * @param _sRightSide2DisplayText
	 *            2nd text for right side
	 * @param _oRight2BaseFont
	 *            Font for 2nd right text
	 * @param _fRight2FontSize
	 *            Font size for 2nd right text
	 * @param _fPositionX1
	 *            X-coordinate for left text
	 * @param _fPositionX2
	 *            X-coordinate for 1st right text
	 * @param _fPositionX3
	 *            X-coordinate for 2nd right text
	 * @param _fPositionY
	 *            Y coordinate for all text
	 * @throws DocumentException
	 */
	public void createDualRightLeftValue(PdfContentByte _oPDFContentByte, String _sLeftSideDisplayText, BaseFont _oLeftBaseFont, float _fLeftFontSize,
			String _sRightSide1DisplayText, BaseFont _oRight1BaseFont, float _fRight1FontSize, String _sRightSide2DisplayText, BaseFont _oRight2BaseFont, float _fRight2FontSize,
			float _fPositionX1, float _fPositionX2, float _fPositionX3, float _fPositionY) throws DocumentException
	{
		...
		_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_RIGHT, _sRightSide1DisplayText, _fPositionX2, _fPositionY, 0);
		...
		if (_sRightSide2DisplayText != null && _sRightSide2DisplayText.length() > 0)
		{
			...
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_RIGHT, _sRightSide2DisplayText, _fPositionX3, _fPositionY, 0);
		}
	...
	/**
	 * This method creates a column that will not go past the specified
	 * coordinate to the right.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _sLeftSideDisplayText_sLeftSideDisplayText
	 *            Text for left side
	 * @param _oLeftBaseFont
	 *            Font for left text
	 * @param _fLeftFontSize
	 ...
	 * @throws DocumentException
	 */
	public void createRightLeftLeaderValueWithFixedRightStop(PdfContentByte _oPDFContentByte, String _sLeftSideDisplayText, BaseFont _oLeftBaseFont, float _fLeftFontSize,
			String _sRightSideDisplayText, BaseFont _oRightBaseFont, float _fRightFontSize, Font _oLeaderFont, float _fPositionX1, float _fPositionX2, float _fPositionY,
			float _fPositionX3) throws DocumentException
	{
		_oPDFContentByte.beginText();
		_oPDFContentByte.setFontAndSize(_oLeftBaseFont, _fLeftFontSize);
		_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sLeftSideDisplayText, _fPositionX1, _fPositionY, 0);
		...
		oPhrase.add("................................................................................................................................................................................................................................................................................................................................................................................................................................................................................................");
		oColumnText.addText(oPhrase);
		oColumnText.setSimpleColumn(_fPositionX1 + fLeftPhraseWidth, _fPositionY + _oLeaderFont.getCalculatedSize(), _fPositionX3, _fPositionY, _oLeaderFont.getSize(),
				Element.ALIGN_LEFT);
		oColumnText.go();
	...
	/**
	 * This method creates a label/value pair for rendering with a maximum
	 * right edge and a custom leader between the label and value.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _sLeftSideDisplayText
	 ...
	 * @param _fRightFontSize
	 *            Font size for right text
	 * @param _sLeaderDisplayText
	 *            Text for leader
	 * @param _oLeaderBaseFont
	 *            Font for leader
	 * @param _fLeaderFontSize
	 *            Font size for leader
	 * @param _oLeaderFont
	 *            Font for leader dots
	 * @param _fPositionX1
	 *            X-coordinate for left text
	 * @param _fPositionX2
	 *            X-coordinate for custom leader
	 * @param _fPositionY
	 *            Y-coordinate for this display
	 * @param _fPositionX3
	 *            X-coordinate for right stop
	 * @param _fPositionX4
	 *            X-coordinate for right text
	 * @throws DocumentException
	 */
	public void createRightLeftLeaderValueWithFixedRightStopAndLeaderText(PdfContentByte _oPDFContentByte, String _sLeftSideDisplayText, BaseFont _oLeftBaseFont,
			float _fLeftFontSize, String _sRightSideDisplayText, BaseFont _oRightBaseFont, float _fRightFontSize, String _sLeaderDisplayText, BaseFont _oLeaderBaseFont,
			float _fLeaderFontSize, Font _oLeaderFont, float _fPositionX1, float _fPositionX2, float _fPositionY, float _fPositionX3, float _fPositionX4) throws DocumentException
	{
		_oPDFContentByte.beginText();
		_oPDFContentByte.setFontAndSize(_oLeftBaseFont, _fLeftFontSize);
		_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sLeftSideDisplayText, _fPositionX1, _fPositionY, 0);
		...
		float fLeaderPhraseWidth = _oPDFContentByte.getEffectiveStringWidth(_sLeaderDisplayText, false) + _fLeaderFontSize / 2;

		_oPDFContentByte.setFontAndSize(_oRightBaseFont, _fRightFontSize);
		_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_RIGHT, _sRightSideDisplayText, _fPositionX4, _fPositionY, 0);
		_oPDFContentByte.endText();
		...
		oPhrase.add("................................................................................................................................................................................................................................................................................................................................................................................................................................................................................................");
		oColumnText.addText(oPhrase);
		oColumnText.setSimpleColumn(_fPositionX1 + fLeftPhraseWidth, _fPositionY + _oLeaderFont.getCalculatedSize(), _fPositionX2, _fPositionY, _oLeaderFont.getSize(),
				Element.ALIGN_LEFT);
		oColumnText.go();
		...
		oColumnText2.setSimpleColumn(_fPositionX2 + fLeaderPhraseWidth, _fPositionY + _oLeaderFont.getCalculatedSize(), _fPositionX3, _fPositionY, _oLeaderFont.getSize(),
				Element.ALIGN_LEFT);
		oColumnText2.go();
	...
	/**
	 * This method writes text to a document with a leader between the left and
	 * right text areas.  In this method both texts (right & left) will be
	 * right aligned.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 ...
	 * @throws DocumentException
	 */
	public void createRightLeftLeaderValueRightJustifiedWithFixedRightStop(PdfContentByte _oPDFContentByte, String _sLeftSideDisplayText, BaseFont _oLeftBaseFont,
			float _fLeftFontSize, String _sRightSideDisplayText, BaseFont _oRightBaseFont, float _fRightFontSize, Font _oLeaderFont, float _fPositionX1, float _fPositionX2,
			float _fPositionY, float _fPositionX3) throws DocumentException
	{
		_oPDFContentByte.beginText();
		_oPDFContentByte.setFontAndSize(_oLeftBaseFont, _fLeftFontSize);
		_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_RIGHT, _sLeftSideDisplayText, _fPositionX1, _fPositionY, 0);
		_oPDFContentByte.setFontAndSize(_oRightBaseFont, _fRightFontSize);
		_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_RIGHT, _sRightSideDisplayText, _fPositionX2, _fPositionY, 0);
		_oPDFContentByte.endText();
		...
		oPhrase.add("................................................................................................................................................................................................................................................................................................................................................................................................................................................................................................");
		oColumnText.addText(oPhrase);
		oColumnText.setSimpleColumn(_fPositionX1 + _fLeftFontSize / 2, _fPositionY + _oLeaderFont.getCalculatedSize(), _fPositionX3, _fPositionY, _oLeaderFont.getSize(),
				Element.ALIGN_LEFT);
		oColumnText.go();
	...
	/**
	 * This method creates a "3 of 9" type bar code image and adds it to the
	 * PDF document.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 ...
	 * @throws DocumentException
	 */
	public void create3of9Barcode(PdfContentByte _oPDFContentByte, String _sDisplayText, float _fPositionX, float _fPositionY) throws DocumentException
	{
		...
		_oPDFContentByte.addImage(oCode39.createImageWithBarcode(_oPDFContentByte, null, null), oRectangle.getWidth(), 0, 0, oRectangle.getHeight(), _fPositionX, _fPositionY);
	}

	/**
	 * This method creates a "3 of 9" type bar code image with a user-defined
	 * height and adds it to the PDF document.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _sDisplayText
	 ...
	 *            Y-coordinate for image
	 * @param _fBarHeight
	 *            Desired height for bar code
	 * @throws DocumentException
	 */
	public void create3of9BarcodeCustomHeight(PdfContentByte _oPDFContentByte, String _sDisplayText, float _fPositionX, float _fPositionY, float _fBarHeight)
			throws DocumentException
	{
		Barcode39 oCode39 = new Barcode39();
		oCode39.setCode(_sDisplayText);
	 ...
	 * @param _fBarHeight
	 *            Desired height for bar code
	 * @param _fRotateDegrees
	 *            Number of degrees to rotate image
	 * @throws DocumentException
	 */
	public void create3of9BarcodeCustomHeightRotateDegrees(PdfContentByte _oPDFContentByte, String _sDisplayText, float _fPositionX, float _fPositionY, float _fBarHeight,
			float _fRotateDegrees) throws DocumentException
	{
		Barcode39 oCode39 = new Barcode39();
		oCode39.setCode(_sDisplayText);
		oCode39.setBarHeight(_fBarHeight);
	...
	/**
	 * This method creates a code 128 type bar code image and adds it to the
	 * PDF document.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 ...
	 * @throws DocumentException
	 */
	public void create128Barcode(PdfContentByte _oPDFContentByte, String _sDisplayText, float _fPositionX, float _fPositionY) throws DocumentException
	{
		...
		_oPDFContentByte.addImage(oCode128.createImageWithBarcode(_oPDFContentByte, null, null), oRectangle.getWidth(), 0, 0, oRectangle.getHeight(), _fPositionX, _fPositionY);
	}

	/**
	 * This method creates a 128 type bar code image with a user-defined
	 * height and adds it to the PDF document.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 ...
	 *            Number of degrees to rotate image
	 * @param _fScale
	 *            Percent to scale the image by
	 * @throws DocumentException
	 */
	public void create128BarcodeCustomHeightRotationScale(PdfContentByte _oPDFContentByte, String _sDisplayText, float _fPositionX, float _fPositionY, float _fBarHeight,
			float _fRotateDegrees, float _fScale) throws DocumentException
	{
		Barcode128 oCode128 = new Barcode128();
		oCode128.setCode(_sDisplayText);
	...
	/**
	 * This method creates a "3 of 9" type bar code image and adds it to the
	 * PDF document- the image will be left aligned.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _sDisplayText
	 ...
	 * @throws DocumentException
	 */
	public void create3of9BarcodeLeftAlign(PdfContentByte _oPDFContentByte, String _sDisplayText, float _fPositionX, float _fPositionY) throws DocumentException
	{
		Barcode39 oCode39 = new Barcode39();
		oCode39.setCode(_sDisplayText);
		oCode39.setBarHeight(22);
	...
	/**
	 * This method creates a "3 of 9" type bar code image with a height of 16
	 ...
	 * @throws DocumentException
	 */
	public void create3of9Barcode16(PdfContentByte _oPDFContentByte, String _sDisplayText, float _fPositionX, float _fPositionY) throws DocumentException
	{
		Barcode39 oCode39 = new Barcode39();
		oCode39.setCode(_sDisplayText);
	...
	/**
	 * This method creates an Interleaved 2 of 5 bar code image with a custom height and adds
	 ...
	 * @throws DocumentException
	 */
	public void createInterleaved2of5BarcodeCustomHeight(PdfContentByte _oPDFContentByte, String _sDisplayText, float _fPositionX, float _fPositionY, float _fBarHeight)
			throws DocumentException
	{
		...
		_oPDFContentByte.addImage(oCode25.createImageWithBarcode(_oPDFContentByte, null, null), oRectangle.getWidth(), 0, 0, oRectangle.getHeight(), _fPositionX, _fPositionY);
	}
	
	
	public void createInterleaved2of5BarcodeRotated(PdfContentByte _oPDFContentByte, String _sDisplayText, float _fPositionX, float _fPositionY, float _fBarHeight,float _fRotateDegrees)
			throws DocumentException
	{
		BarcodeInter25 oCode25 = new BarcodeInter25();
		oCode25.setCode(_sDisplayText);
	...
	/**
	 * This method creates an Interleaved 2 of 5 bar code image and adds it to
	 * the PDF document.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 ...
	 * @throws DocumentException
	 */
	public void createInterleaved2of5Barcode(PdfContentByte _oPDFContentByte, String _sDisplayText, float _fPositionX, float _fPositionY) throws DocumentException
	{
		createInterleaved2of5BarcodeCustomHeight(_oPDFContentByte, _sDisplayText, _fPositionX, _fPositionY, 22);
	}

	/**
	 * This method creates an OCR scan line image and adds it to the PDF
	 * aligned right.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _sPathToOCRFont
	 ...
	 * @throws IOException
	 */
	public void createOCRScanlineRight(PdfContentByte _oPDFContentByte, String _sPathToOCRFont, String _sDisplayText, float _fPositionX, float _fPositionY)
	 ...
	 * @throws IOException
	 */
	public void createOCRScanlineLeft(PdfContentByte _oPDFContentByte, String _sPathToOCRAFont, String _sDisplayText, float _fPositionX, float _fPositionY)
			throws DocumentException, IOException
	{
	...
	/**
	 * This method creates a bar code to be read by O'Brien's multi-tray inserter to determine what will go into each envelope.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _bCloseEnvelopeFlag 
	 * @param _sRenderMode
	 *            Render mode (e.g. TEST for testing mode)
	 * @param _sBarcodeContent
	 *            Content to be rendered as bar code
	 * @throws DocumentException
	 */
	public void doOBrienBarCodeStandardLocation(PdfContentByte _oPDFContentByte, double _dPageShimmyLeftRight, double _dPageShimmyUpDown, boolean _bCloseEnvelopeFlag,
			int _nGroupSequence, int _nPageRollSequence, boolean _bHas9Envelope, boolean _bHasItemInPocketOne, boolean _bHasItemInPocketTwo, boolean _bHasItemInPocketThree,
			boolean _bHasItemInPocketFour, boolean _bHasItemInPocketSix, boolean _bUseInkMark, String _sRenderMode) throws DocumentException
	{
		doOBrienBarCode(_oPDFContentByte, XN_OBRIEN_BARCODE_XPOS, XN_OBRIEN_BARCODE_YPOS, _dPageShimmyLeftRight, _dPageShimmyUpDown, _bCloseEnvelopeFlag, _nGroupSequence,
				_nPageRollSequence, _bHas9Envelope, _bHasItemInPocketOne, _bHasItemInPocketTwo, _bHasItemInPocketThree, _bHasItemInPocketFour, _bHasItemInPocketSix, _bUseInkMark,
				_sRenderMode);
	}

	public void doOBrienBarCodeStandardLocationVehicle(PdfContentByte _oPDFContentByte, double _dPageShimmyLeftRight, double _dPageShimmyUpDown, String _sCloseEnvelopeFlag,
			int _nGroupSequence, int _nPageRollSequence, String _sHas9Envelope, String _sHasItemInPocketOne, String _sHasItemInPocketTwo, String _sHasItemInPocketThree,
			String _sHasItemInPocketFour, String _sHasItemInPocketSix, boolean _bUseInkMark, String _sRenderMode) throws DocumentException
	{
		...
		doOBrienBarCodeStandardLocation(_oPDFContentByte, _dPageShimmyLeftRight, _dPageShimmyUpDown, _bCloseEnvelopeFlag, _nGroupSequence, _nPageRollSequence, _bHas9Envelope,
				_bHasItemInPocketOne, _bHasItemInPocketTwo, _bHasItemInPocketThree, _bHasItemInPocketFour, _bHasItemInPocketSix, _bUseInkMark, _sRenderMode);
	}
	 ...
	 *            PDF object where content will be rendered
	 * @param _nStartXPos
	 *            X-coordinate to begin rendering bar code
	 * @param _nStartYPos
	 *            Y-coordinate to begin rendering bar code
	 * @param _bCloseEnvelopeFlag 
	 * @param _sRenderMode
	 *            Render mode (e.g. TEST for testing mode- will print human-readable bar code content)
	 * @param _sBarcodeContent
	 *            Content to be rendered as bar code
	 * @throws DocumentException
	 */
	public void doOBrienBarCode(PdfContentByte _oPDFContentByte, int _nStartXPos, int _nStartYPos, double _dPageShimmyLeftRight, double _dPageShimmyUpDown,
			boolean _bCloseEnvelopeFlag, int _nGroupSequence, int _nPageRollSequence, boolean _bHas9Envelope, boolean _bHasItemInPocketOne, boolean _bHasItemInPocketTwo,
			boolean _bHasItemInPocketThree, boolean _bHasItemInPocketFour, boolean _bHasItemInPocketSix, boolean _bUseInkMark, String _sRenderMode) throws DocumentException
	{
		String sBarcodeContent = getOBrienBarcode(_bCloseEnvelopeFlag, _nGroupSequence, _nPageRollSequence, _bHas9Envelope, _bHasItemInPocketOne, _bHasItemInPocketTwo,
		...
		create128BarcodeCustomHeightRotationScale(_oPDFContentByte, sBarcodeContent, nAbsStartXPos, nAbsStartYPos, 15, 90, 100);

		if (_sRenderMode.equalsIgnoreCase("TEST"))
		{
			_oPDFContentByte.beginText();
			_oPDFContentByte.setFontAndSize(ioBaseFontHelveticaRegular, 6);
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_CENTER, sBarcodeContent, nAbsStartXPos - 3, nAbsStartYPos + 25, 90);
		...
		else
		{
			...
			String sHugsAndKissesLine = sHasItemInPocketOne + " " + sHasItemInPocketTwo + " " + sHasItemInPocketThree + " " + sHasItemInPocketFour + " " + sHas9Envelope + " "
					+ sHasItemInPocketFive + "   " + sCloseEnvelopeFlag;

			_oPDFContentByte.beginText();
			_oPDFContentByte.setFontAndSize(ioBaseFontHelveticaRegular, 6);
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_CENTER, sHugsAndKissesLine, nAbsStartXPos - 7, nAbsStartYPos + 28, 270);
	...
	public String getOBrienBarcode(boolean _bCloseEnvelopeFlag, int _nGroupSequence, int _nPageRollSequence, boolean _bHas9Envelope, boolean _bHasItemInPocketOne,
			boolean _bHasItemInPocketTwo, boolean _bHasItemInPocketThree, boolean _bHasItemInPocketFour, boolean _bHasItemInPocketSix, boolean _bUseInkMark)
	{
		...
		oBarcodeContent.append(getOBrienInserterSequence(_bHas9Envelope, _bHasItemInPocketOne, _bHasItemInPocketTwo, _bHasItemInPocketThree, _bHasItemInPocketFour,
				_bHasItemInPocketSix, _bUseInkMark));

		return oBarcodeContent.toString();
	}

	public String getOBrienInserterSequence(boolean _bHas9Envelope, boolean _bHasItemInPocketOne, boolean _bHasItemInPocketTwo, boolean _bHasItemInPocketThree,
			boolean _bHasItemInPocketFour, boolean _bHasItemInPocketSix, boolean _bUseInkMark)
	{
		...
		/*
		 * Currently, we do not use the inkmark.  If we need to add the inkmark, here is the spec from O'Brien:
		 * 0 = No insert no inkmark
		 * 1 = Insert in the 6th station and no inkmark
		 * G = No insert with an inkmark
		 * H = Insert with an inkmark
		 */
	 ...
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _bHas9Envelope
	 *            Flag if #9 envelope should be included
	 * @param _bHasItemInPocketOne
	 *            Flag if an item should be included in pocket 1
	 * @param _bHasItemInPocketTwo
	 *            Flag if an item should be included in pocket 2
	 * @param _bHasItemInPocketThree
	 *            Flag if an item should be included in pocket 3
	 * @param _bHasItemInPocketFour
	 *            Flag if an item should be included in pocket 4
	 * @param _bHasItemInPocketFive
	 *            Flag if an item should be included in pocket 5
	 * @param _sRenderMode
	 *            Render mode (e.g. TEST for testing mode)
	 * @param _asQBCodes
	 *            String array used in TEST mode for labeling each mark
	 */
...
//	}
	 ...
	 *            PDF object where content will be rendered
	 * @param _nStartXPos
	 *            X-coordinate to begin rendering marks
	 * @param _nStartYPos
	 *            Y-coordinate to begin rendering marks
	 * @param _bHas9Envelope
	 *            Flag if #9 envelope should be included
	 * @param _bHasItemInPocketOne
	 *            Flag if an item should be included in pocket 1
...
//	}
	 ...
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _sPathToIMBFont
	 *            Path to font file
	 * @param _sRecordSequence
	 *            Sequence to render
	 * @param _sAddresseeName
	 *            Name to mail to
	 * @param _sAddress1
	 *            Address line 1
	 * @param _sAddress2
	 ...
	 * @param _sZipPlus4
	 *            Zip Plus4 code for mailing
	 * @param _sZipDP
	 *            NOT USED
	 * @param _sEndorsement
	 *            Endorsement to add to the address block
	 * @param _sIMB
	 *            String to render as IMB
	 * @param _oBaseFont
	 *            Font to use for printing each non-IMB element
	 * @param _sStockNumber
	 *            Stock number for print run
	 * @throws DocumentException
	 * @throws IOException
	 */
	public void createAddresseeBlockIMB(PdfContentByte _oPDFContentByte, String _sPathToIMBFont, String _sRecordSequence, String _sAddresseeName, String _sAddress1,
			String _sAddress2, String _sAddress3, String _sAddress4, String _sCity, String _sState, String _sZip, String _sZipPlus4, String _sZipDP, String _sEndorsement,
			String _sIMB, BaseFont _oBaseFont, String _sStockNumber) throws DocumentException, IOException
	{
		createAddresseeBlockIMB(_oPDFContentByte, _sPathToIMBFont, _sRecordSequence, null, _sAddresseeName, null, _sAddress1, _sAddress2, _sAddress3, _sAddress4, _sCity, _sState,
				_sZip, _sZipPlus4, _sZipDP, _sEndorsement, _sIMB, _oBaseFont, 80, 45, _sStockNumber, false, null);
	}

	/**
	 * This method prints the addressee lines with the IMB record sequence (for utility clients with two addressee names).
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _sPathToIMBFont
	 ...
	 * @param _sRecordSequence
	 *            Sequence to render
	 * @param _sAddresseeName1
	 *            Name to mail to
	 * @param _sAddresseeName2
	 *            2nd name to mail to
	 * @param _sAddress1
	 *            Address line 1
	 * @param _sAddress2
	 ...
	 * @throws IOException
	 */
	public void createAddresseeBlockIMBUtilityTwoNames(PdfContentByte _oPDFContentByte, String _sPathToIMBFont, String _sRecordSequence, String _sAddresseeName1,
			String _sAddresseeName2, String _sAddress1, String _sAddress2, String _sAddress3, String _sAddress4, String _sCity, String _sState, String _sZip, String _sZipPlus4,
			String _sZipDP, String _sEndorsement, String _sIMB, BaseFont _oBaseFont, String _sStockNumber) throws DocumentException, IOException
	{
		createAddresseeBlockIMB(_oPDFContentByte, _sPathToIMBFont, _sRecordSequence, null, _sAddresseeName1, _sAddresseeName2, _sAddress1, _sAddress2, _sAddress3, _sAddress4,
				_sCity, _sState, _sZip, _sZipPlus4, _sZipDP, _sEndorsement, _sIMB, _oBaseFont, 80, 45, _sStockNumber, false, null);
	}

	/**
	 * This method prints the addressee lines with the IMB record sequence and
	 * OCR-A sequence.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 ...
	 *            Stock number for print run
	 * @param _sPathToOCRAFont
	 *            Path to find OCR-A text font
	 * @throws DocumentException
	 * @throws IOException
	 */
	public void createAddresseeBlockIMBWithRecordSequence(PdfContentByte _oPDFContentByte, String _sPathToIMBFont, String _sRecordSequence, String _sAddresseeName,
			String _sAddress1, String _sAddress2, String _sAddress3, String _sAddress4, String _sCity, String _sState, String _sZip, String _sZipPlus4, String _sZipDP,
			String _sEndorsement, String _sIMB, BaseFont _oBaseFont, String _sStockNumber, String _sPathToOCRAFont) throws DocumentException, IOException
	{
		createAddresseeBlockIMB(_oPDFContentByte, _sPathToIMBFont, _sRecordSequence, null, _sAddresseeName, null, _sAddress1, _sAddress2, _sAddress3, _sAddress4, _sCity, _sState,
				_sZip, _sZipPlus4, _sZipDP, _sEndorsement, _sIMB, _oBaseFont, 80, 45, _sStockNumber, true, _sPathToOCRAFont);
	}
	 ...
	 * @param _sRecordSequence
	 *            Sequence to render
	 * @param _sAlternateRecordSequence
	 *            Alternate record to render
	 * @param _sAddresseeName1
	 *            First Name to mail to
	 * @param _sAddresseeName2
	 *            Second Name to mail to
	 * @param _sAddress1
	 *            Address line 1
	 * @param _sAddress2
	 *            Address line 2
	 ...
	 * @param _oBaseFont
	 *            Font to use for printing each non-IMB element
	 * @param _nLeftEdgeOfPrintElements
	 *            X-coordinate for left edge of printed elements
	 * @param _fPostnetYPos
	 *            Y-coordinate for printed elements
	 * @param _sStockNumber
	 *            Stock number for print run
	 * @param _bPrintRecordSequence
	 *            Flag if record sequence should be printed
	 * @param _sPathToOCRAFont
	 *            Path to find OCR-A text font
	 * @throws DocumentException
	 * @throws IOException
	 */
	public void createAddresseeBlockIMB(PdfContentByte _oPDFContentByte, String _sPathToIMBFont, String _sRecordSequence, String _sAlternateRecordSequence,
			String _sAddresseeName1, String _sAddresseeName2, String _sAddress1, String _sAddress2, String _sAddress3, String _sAddress4, String _sCity, String _sState,
			String _sZip, String _sZipPlus4, String _sZipDP, String _sEndorsement, String _sIMB, BaseFont _oBaseFont, int _nLeftEdgeOfPrintElements, float _fPostnetYPos,
			String _sStockNumber, boolean _bPrintRecordSequence, String _sPathToOCRAFont) throws DocumentException, IOException
	{
		...
		_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, sCityStateZipLine, _nLeftEdgeOfPrintElements, nYPos, 0);
		...
		{
			nYPos = nYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddress1.trim(), _nLeftEdgeOfPrintElements, nYPos, 0);
		...
		{
			nYPos = nYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddress2.trim(), _nLeftEdgeOfPrintElements, nYPos, 0);
		...
		{
			nYPos = nYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddress3.trim(), _nLeftEdgeOfPrintElements, nYPos, 0);
		...
		{
			nYPos = nYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddress4.trim(), _nLeftEdgeOfPrintElements, nYPos, 0);
		...
		{
			nYPos = nYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddresseeName2.trim(), _nLeftEdgeOfPrintElements, nYPos, 0);
		}

		if (_sAddresseeName1 != null && _sAddresseeName1.length() > 0)
		{
			nYPos = nYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddresseeName1, _nLeftEdgeOfPrintElements, nYPos, 0);
		}

		if (_sEndorsement != null && _sEndorsement.length() > 0)
		{
			nYPos = nYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sEndorsement, _nLeftEdgeOfPrintElements, nYPos, 0);
		}
		...
		if (_bPrintRecordSequence)
		{
			...
			if (_sAlternateRecordSequence != null)
			{
				if (_sAlternateRecordSequence.length() > 0)
				{
					...
					_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_RIGHT, _sAlternateRecordSequence, nOriginalLeftEdgeOfPrintElements, _fPostnetYPos + 53, 90);
					_oPDFContentByte.endText();
			...
			else
			{
				...
				String sRecordSequence = asRecordSequence[asRecordSequence.length - 1].replaceAll("\\(", "").replaceAll("\\)", "");
				...
				if (sRecordSequence.length() > 0)
				{
					...
					_oPDFContentByte.beginText();
					_oPDFContentByte.setFontAndSize(oBaseFontOCRA, 10);
					_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_RIGHT, sRecordSequence, nOriginalLeftEdgeOfPrintElements, _fPostnetYPos + 53, 90);
					_oPDFContentByte.endText();
		...
		if (_sIMB.length() > 0)
		{
			...
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sIMB, _nLeftEdgeOfPrintElements, (_fPostnetYPos + 4), 0);
			_oPDFContentByte.endText();
	...
	/**
	 * This method determines the proper postal layout based on the country and renders the
	 * postal information as needed.
	 * 
	 * @param _oPDFContentByte
	 *            PDF object where content will be rendered
	 * @param _sCountry
	 ...
	 * @throws IOException
	 */
	public void createInternationalAddressBlock(PdfContentByte _oPDFContentByte, String _sCountry, String _sAddresseeName1, String _sAddresseeName2, String _sAddress1,
			String _sAddress2, String _sAddress3, String _sAddress4, String _sCity, String _sState, String _sPostalCode, String _sRecordSequence, BaseFont _oBaseFont,
			String _sStockNumber) throws DocumentException, IOException
	{
		if (_sCountry != null && _sCountry.equalsIgnoreCase("CANADA"))
		{
			createCanadianAddressBlock(_oPDFContentByte, _sAddresseeName1, _sAddresseeName2, _sAddress1, _sAddress2, _sAddress3, _sAddress4, _sCity, _sState, _sPostalCode,
					_sRecordSequence, _oBaseFont, _sStockNumber);
		}
	}
	 ...
	 * @throws IOException
	 */
	public void createCanadianAddressBlock(PdfContentByte _oPDFContentByte, String _sAddresseeName1, String _sAddresseeName2, String _sAddress1, String _sAddress2,
			String _sAddress3, String _sAddress4, String _sCity, String _sState, String _sPostalCode, String _sRecordSequence, BaseFont _oBaseFont, String _sStockNumber)
			throws DocumentException, IOException
	{
		...
		{
			fYPos += 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, sCityStateZipLine, nLeftEdgeOfPrintElements, fYPos, 0);
		}
		...
		{
			fYPos = fYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddress1.trim(), nLeftEdgeOfPrintElements, fYPos, 0);
			oAddressLineArrayList.add(_sAddress1.trim());
		}
		...
		{
			fYPos = fYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddress2.trim(), nLeftEdgeOfPrintElements, fYPos, 0);
			oAddressLineArrayList.add(_sAddress2.trim());
		}
		...
		{
			fYPos = fYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddress3.trim(), nLeftEdgeOfPrintElements, fYPos, 0);
			oAddressLineArrayList.add(_sAddress3.trim());
		}
		...
		{
			fYPos = fYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddress4.trim(), nLeftEdgeOfPrintElements, fYPos, 0);
			oAddressLineArrayList.add(_sAddress4.trim());
		}
		...
		{
			fYPos = fYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddresseeName2.trim(), nLeftEdgeOfPrintElements, fYPos, 0);
		}

		if (_sAddresseeName1 != null && _sAddresseeName1.length() > 0)
		{
			fYPos = fYPos + 10;
			_oPDFContentByte.showTextAligned(PdfContentByte.ALIGN_LEFT, _sAddresseeName1, nLeftEdgeOfPrintElements, fYPos, 0);
		}
	 ...
	 * 
	 * @param _fCoordinateX
	 *            Number of points from the right side of the page to render
	...
	/**
	 * This method calculates the absolute Y-coordinate calculated form the top
	 * of the page. 
	 * 
	 * @param _fCoordinateY
	 *            Number of points from the top of the page
	...
	/**
	 * This method returns the calculated absolute Y-coordinate centered
	 * vertically from the top of the page. 
	 * 
	 * @param _fDownAdjust
	 *            Number of points to adjust down from center
	 * @return Absolute Y-coordinate
	 */
	 ...
	 * 
	 * @param _fRightAdjust
	 *            Number of points to adjust right from center
	 * @return Absolute X-coordinate
	 */
	 ...
	 * @param _bLeftJustify True: Text is left justified; False: Text is centered
	 * @param _bTopAlign True: Text is Top aligned; False: Text is centered
	 * @param _fExtraSpacing Amount of Extra Line spacing (0: Single Spaced; 1: double spaced;...)
	 */
	public void generateMessageCenter(List<String> _lsMessageText, Font _oFont, float _fLeft, float _fTop, float _fWidth, float _fHeight, boolean _bShowBoundingBox,
			boolean _bLeftJustify, boolean _bTopAlign, float _fExtraSpacing)
	{
...
//	}
	 ...
	 * 
	 * @param _nPRSeq
	 *    Integer value of the current page roll sequence number.
	 * @return
	 *    The next page roll sequence number based on the O'Brien specification.
	 */
	...
	/**
	 * This method calculates the next group sequence number based on the O'Brien specification.
	 * 
	 * @param _nGrpSeq
	 *    Integer value of the current group sequence number.
	 * @return
	 *    The next group sequence number based on the O'Brien specification.
	 */
	...
	public void createPieChart(String _sTitle, LinkedHashMap<String, Double> _oDataSet, float _fLeft, float _fTop, int _nWidth, int _nHeight) throws IOException, DocumentException
	{
	...
	public void setPaperTray(String _sFormType, BaseColor _oPrintColor)
	{
		...
		getPDFContentByte().showTextAligned(PdfContentByte.ALIGN_CENTER, "FORM::" + _sFormType, 12, PageSize.LETTER.getHeight() / 2, 90);
	...
	public void setPaperTrayCustom(String _sCustomCopy, BaseColor _oPrintColor)
	{
		...
		getPDFContentByte().showTextAligned(PdfContentByte.ALIGN_CENTER, _sCustomCopy, 12, PageSize.LETTER.getHeight() / 2, 90);
		getPDFContentByte().endText();
		getPDFContentByte().restoreState();
	}
